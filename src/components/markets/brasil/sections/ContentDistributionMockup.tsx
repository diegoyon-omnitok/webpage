  ...
  return (
    ...
    >
      ...
      <div className={`flex ${t.pdp}`}>
        <div className={`${t.gallery} flex-shrink-0 flex flex-col gap-1`}>
          <div className={`relative ${t.galleryImg} rounded bg-gray-50 overflow-hidden border border-gray-100`}>
            <Image src={TV_HERO} alt="Smart TV OLED 55 polegadas" fill className="object-contain" />
            ...
            <div className="flex gap-0.5">
              {thumbs.map((thumb, i) => (
                <div key={i} className={`${t.thumb} border overflow-hidden bg-gray-50 ${i === 0 ? "ring-1 ring-pink-500 border-pink-500" : "border-gray-200"}`}>
                  <Image src={thumb} alt="" aria-hidden width={24} height={20} className="w-full h-full object-contain" />
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-0.5">
          ...
          <div className="flex items-baseline gap-1 mt-0.5 leading-none">
            <span className={`${t.oldPrice} text-gray-400 line-through`}>R$ 4.499,90</span>
            <span className={`${t.discount} font-black rounded text-white`} style={{ background: "#DC2626" }}>-22%</span>
          </div>
          <span className={`${t.price} font-black text-gray-900 leading-none`}>R$ 3.499,90</span>
          <span className={`${t.installments} text-emerald-600 font-semibold leading-none`}>10x sem juros</span>
          <div className="flex items-center gap-1 mt-0.5">
            <button className={`flex-1 inline-flex items-center justify-center ${t.cta} font-bold rounded text-white shadow-sm`} style={{ background: "#FF177B" }}>
              <ShoppingCart size={t.ctaIcon} /> {compact ? "Adicionar" : "Adicionar ao carrinho"}
            </button>
            <button className={`${t.heartBtn} rounded border border-gray-200 flex items-center justify-center bg-white flex-shrink-0`} aria-label="Lista de desejos">
          ...
          {!compact && (
            <div className="mt-1 space-y-0.5">
              <div className={`flex items-center ${t.trust}`}>
                ...
                <span className="text-gray-600">Frete grátis em 2-3 dias</span>
              </div>
              <div className={`flex items-center ${t.trust}`}>
                <Shield size={t.trustIcon} className="text-emerald-600" />
                <span className="text-gray-600">Garantia oficial de 1 ano</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className={`${t.specsWrap} border-t border-gray-100 bg-gray-50`}>
        <p className={`${t.specsTitle} font-bold text-gray-700 uppercase tracking-wider`}>Especificações</p>
      ...
      {/* Ficha de conteúdo — rich multi-block module, emphasizing enriched content */}
      <div className="relative border-t border-gray-100">
        <div className={`absolute ${t.fichaBadge.split(" ").slice(2).join(" ")} z-20`}>
          <span className={`inline-flex items-center gap-0.5 rounded-full ${t.fichaBadge.split(" ").slice(0, 2).join(" ")} font-bold text-white shadow-md`} style={{ background: "linear-gradient(90deg, #FF177B 0%, #4D4A9D 100%)" }}>
            <Sparkles size={t.fichaBadgeIcon} /> {compact ? "Enriquecido" : "Conteúdo enriquecido"}
          </span>
        </div>
        ...
        <div className={`grid grid-cols-2 ${compact ? "p-0.5 pt-1 gap-0.5" : "p-1.5 pt-2 gap-1.5"}`}>
          {/* Cell 1: image + badge (Gama de cores) */}
          <div className={`relative rounded overflow-hidden bg-black ${compact ? "h-6" : "h-16"}`}>
            <Image src={LIFESTYLE} alt="Lifestyle com o televisor" fill className="object-cover" />
            <div className={`absolute inline-flex items-center rounded-full bg-black/60 backdrop-blur ${compact ? "top-px left-px px-0.5 py-px gap-px" : "top-1 left-1 px-1.5 py-0.5 gap-0.5"}`}>
              ...
              <span className={`font-bold text-white tracking-wider ${compact ? "text-[2.5px]" : "text-[5px]"}`}>GAMA DE CORES</span>
            </div>
          ...
          {/* Cell 2: text — Cores que ganham vida */}
          <div className={`bg-white rounded flex flex-col justify-center ${compact ? "h-6 px-0.5 py-0.5" : "h-16 px-1 py-1"}`}>
            ...
            <h4 className={`font-black text-gray-900 leading-tight ${compact ? "text-[3.5px]" : "text-[8px]"}`}>Cores que ganham vida</h4>
            {!compact && (
              <p className="text-gray-500 text-[5px] leading-snug mt-0.5">
                Pretos absolutos e mais de um bilhão de cores. Cada pixel se ilumina de forma independente.
              </p>
            )}
          </div>

          {/* Cell 3: text — Som que envolve você */}
          <div className={`bg-white rounded flex flex-col justify-center ${compact ? "h-6 px-0.5 py-0.5" : "h-16 px-1 py-1"}`}>
            <span className={`font-black tracking-widest leading-none ${compact ? "text-[2.5px]" : "text-[6px]"}`} style={{ color: "#FF177B" }}>02</span>
            <h4 className={`font-black text-gray-900 leading-tight ${compact ? "text-[3.5px]" : "text-[8px]"}`}>Som que envolve você</h4>
            {!compact && (
              <p className="text-gray-500 text-[5px] leading-snug mt-0.5">
                Dolby Atmos integrado com 40W. O áudio acompanha a ação na tela.
              </p>
            )}
          </div>
          ...
          <div className={`relative rounded overflow-hidden bg-gray-900 ${compact ? "h-6" : "h-16"}`}>
            <Image src={TV_THUMB_4} alt="Televisor com Dolby Atmos" fill className="object-cover" />
            <div className={`absolute inline-flex items-center rounded-full bg-black/60 backdrop-blur ${compact ? "top-px left-px px-0.5 py-px gap-px" : "top-1 left-1 px-1.5 py-0.5 gap-0.5"}`}>
              <span className={`rounded-full ${compact ? "w-px h-px" : "w-1 h-1"}`} style={{ background: "#FF177B" }} />
              <span className={`font-bold text-white tracking-wider ${compact ? "text-[2.5px]" : "text-[5px]"}`}>REPRODUZINDO</span>
            </div>
          ...
          {/* Full-width: Vista técnica / Design ultrafino — master only */}
          {!compact && (
            <div className="col-span-2 relative h-20 rounded overflow-hidden">
              <Image src={LIFESTYLE_TECH} alt="Vista técnica do televisor ultrafino" fill className="object-cover" />
              ...
              <div className="absolute bottom-1 left-1.5 right-1.5 flex items-end justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <h4 className="text-[8px] font-black text-white leading-tight">Design ultrafino</h4>
                  <p className="text-[5px] text-white/80 leading-snug">Apenas 5mm de perfil. Bordas quase invisíveis.</p>
                </div>
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4">
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <p className="text-[7px] font-bold text-gray-400 uppercase tracking-wider">Distribuição automática</p>
      </div>
      ...
      <div className="text-center">
        <p className="text-[11px] font-black tabular-nums" style={{ color: "#FF177B" }}>
          {published.length}/{retailers.length} varejistas atualizados
        </p>
        <p className="text-[8px] text-gray-400">
          Mais de 150 varejistas conectados na rede da Omnitok
        </p>
      </div>
    </div>
  );
